...
export class CostCenterModel {
  ...
  static async create(dados: NovoCentroCusto): Promise<CostCenter> {
    return comAtividade(
      ...
      (criado) => ({
        module: "FINANCEIRO",
        description: `Centro de custo cadastrado — ${criado.name}`,
      })
    );
  }

  static async update(id: string, dados: AtualizacaoCentroCusto): Promise<void> {
    const [atual] = await (await getDatabase()).select<{ name: string }[]>("SELECT name FROM cost_centers WHERE id = $1", [id]);
    ...
    return comAtividade(
      ...
      () => ({
        module: "FINANCEIRO",
        description: `Centro de custo ${verboAtualizacao(dados, "o")} — ${dados.name ?? atual?.name ?? id}`,
      })
    );
  }
}
