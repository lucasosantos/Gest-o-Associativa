...
import { ReceivableModel, type ReceivableInstallment } from "../models/Receivable.js";
...
const restante =
  ...
  props.installment.received_amount;
...
async function handleSubmit() {
  ...
  erro.value = "";
  try {
    await ReceivableModel.receiveInstallment(props.installment.id, {
...
<template>
  ...
  <form v-else class="payment-form" @submit.prevent="handleSubmit">
    ...
    <p class="context">
      Parcela {{ installment.installment_number }} de <strong>{{ receivableDescription }}</strong> — saldo restante
    ...
    <template v-else>
      ...
      <div class="field">
        <label class="field-label" for="payment-date">Data do recebimento *</label>
        <input id="payment-date" v-model="paymentDate" type="date" :disabled="saving" required />
      </div>

      <div class="field">
        <label class="field-label" for="amount">Valor recebido (R$) *</label>
