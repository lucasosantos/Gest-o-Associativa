...
import { ref } from "vue";
import { CostCenterModel } from "../models/CostCenter.js";
...
async function handleSubmit() {
  if (!name.value.trim()) {
    erro.value = "Informe o nome do centro de custo.";
    return;
  }
  ...
  erro.value = "";
  try {
    await CostCenterModel.create({ name: name.value.trim(), code: code.value.trim() || null });
    props.onSaved?.();
    closeModal();
  } catch (error) {
...
<template>
  <form class="cost-center-form" @submit.prevent="handleSubmit">
